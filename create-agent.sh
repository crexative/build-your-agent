...
load_strings() {
  if [[ "$LANG_CODE" == "es" ]]; then
    ...
    MSG_MODEL_SONNET="Mejor para código y tareas generales (recomendado)"
    MSG_MODEL_OPUS="Mejor para razonamiento profundo, orquestación, arquitectura"
    MSG_MODEL_HAIKU="Mejor para tareas ligeras, alto volumen, respuestas rápidas"
    ...
    MSG_MCP_CONFIRM="¿Usará herramientas MCP externas?"
    MSG_MCP_TIP="Ej: github, filesystem, postgres, supabase"
    MSG_MCP_NAMES="Nombres MCP separados por coma (ej: github, postgres):"
    ...
    MSG_MODEL_SONNET="Best for coding and general tasks (recommended)"
    MSG_MODEL_OPUS="Best for deep reasoning, orchestration, architectural decisions"
    MSG_MODEL_HAIKU="Best for lightweight, high-volume, fast-response tasks"
    ...
    MSG_MCP_CONFIRM="Will it use external MCP tools?"
    MSG_MCP_TIP="e.g. github, filesystem, postgres, supabase"
    MSG_MCP_NAMES="MCP tool names separated by commas (e.g. github, postgres):"
...
show_summary() {
  ...
  echo -e "  ${DIM}curl -fsSL https://raw.githubusercontent.com/crexative/build-your-agent/main/install.sh | bash${RESET}"
  echo ""
